...
main() {
  ...
  echo "Running Besu dev node with Firehose tracer activated via 'fireeth'"
  FIREHOSE_VERSION="$tracer_version" FORK_VERSION="$fork_version" run_fireeth 0 "bash" "$ROOT/wrapped_besu_dev.sh"
}

usage() {
  echo "Usage: $0 <tracer-version> [<fork-version>]"
  echo ""
  echo "Runs a Besu dev with the Firehose tracer activated using version <tracer-version>"
  ...
  echo ""
  echo "Note that the fork version might or might not be supported based on the 'besu' binary"
  echo "you are using."
}
