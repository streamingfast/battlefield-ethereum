...
async function main() {
    ...
    await okSend(mainContract.methods.nativeTransfer("0xd549d2fd4b177767b84ab2fd17423cee1cf1d7bd"), {
        from: defaultAddress, value: "0x1234"
    })

    // Transfer to new address
    await okSend(mainContract.methods.nativeTransfer("0x0000000000000000000000000000000000002002"), {
        from: defaultAddress, value: "0x5678"
    })

    // Nested transfer to existing address
    await okSend(mainContract.methods.nestedNativeTransfer(childContract.address, "0xd549d2fd4b177767b84ab2fd17423cee1cf1d7bd"), {
        from: defaultAddress, value: "0x9abc"
    })

    // Nested transfer to new address
    await okSend(mainContract.methods.nestedNativeTransfer(childContract.address, "0x0000000000000000000000000000000000003003"), {
        from: defaultAddress, value: "0xdeff"
    })

    console.log("Performing 'call' transactions")
    await okSend(mainContract.methods.completeCallTree(childContract.address, grandChildContract.address))
    ...
    await okSend(mainContract.methods.deepNestedCallForLowestGas(childContract.address, grandChildContract.address))
    await okSend(mainContract.methods.deepNestedLowGas(childContract.address, grandChildContract.address))
    ...
    await okSend(mainContract.methods.longStringInput("realy long string larger than 32 bytes to test out solidity splitting stuff"))
...
async function koSend(trx: any, sendOptions?: SendOptions) {
    ...
    if (receipt.gasUsed != sendOptions!.gas) {
        console.log(`Transaction '${receipt.transactionHash}' expected to failed, but succeed?`, receipt, sendOptions!.gas)
